...
export default class IndividualMangaScreen extends Component {
    ...
    addUserMedia() {
        let url = `http://localhost:8080/api/add_user_media?user_id=${this.state.userId}&media_id=${this.state.manga.id}`;
    ...
    addMediaToFavourites() {
        let url = `http://localhost:8080/api/add_media_to_favourites?username=${this.state.username}&title=${this.state.media.title}`;
    ...
    componentDidMount(){
        let url = `http://localhost:8080/api/get_manga_by_id?id=${this.state.id}`;
